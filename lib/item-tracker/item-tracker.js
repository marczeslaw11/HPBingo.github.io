const counterComponent = ({ denominator, icon }, matches) => {
  ...
  button.innerHTML = `<img class=counter-icon src="./lib/item-tracker/icons/${icon}.png">`;
...
const tokensComponent = ({ rows, icon }, matches) => {
  ...
  rows.forEach(icons => {
    ...
    icons.forEach(icon => {
      ...
      checkbox.innerHTML = `<input type="checkbox"><img class="token-icon" src="./lib/item-tracker/icons/${filename}.png">`;
